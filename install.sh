...
if [ -d "$DATA_DIR/doris-fe" ] && [ "$(ls -A "$DATA_DIR/doris-fe" 2>/dev/null)" ]; then
    ...
    if [[ "$clean_resp" =~ ^[Yy]$ ]]; then
        ...
        # Migrate DuckDB from old Docker named volume to new bind mount location
        # Uses a temp Alpine container so it works on all platforms (Linux, macOS, WSL)
        if $DOCKER_CMD volume inspect xd-oss-stack_app-duckdb >/dev/null 2>&1; then
            if [ ! -f "$DATA_DIR/app/app.duckdb" ]; then
                ...
                if [ -f "$DATA_DIR/app/app.duckdb" ]; then
                    ...
                    warn "Migration failed — app will create fresh database"
                fi
            fi
        fi
    fi
...
case "$CMD" in
  ...
  update)
    ...
    # Also update be-entrypoint.sh from latest installer
